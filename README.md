Creamtank Protocol
==================
Forked from Compound commit XXXX

Changelog

Creamtroller
* similar to Comptroller but adapted for Joe's RewardDistributor contract
* other edits too

CreamtrollerStorage
* got rid of all the COMP rewards etc. to make room for reward distributor things
* Very similar to Joe's JoeTrollerStorage. 

GovernorBravorDelegateG2
* Same but _initiate() is slightly different because we have no alpha migration

Lens
* Removed anything related to GovAlpha
* Similar to JoetrollerLens

RewardDistributor
* This takes over all the COMP rewards that are baked into the Comptroller etc.

JumpRateModelV2
* blocksPerYear adjusted for Milkomeda

Oracle
* Has an admin and poster now
...
Identical But With Name Changes
* Unitroller
* CErc20
* CErc20Delegate
* CErc20Delegator
* CToken
* CreamtrollerInterface
* CTokenInterfaces
* ErrorReporter
* Ctank
...
New Files
* TimelockInterface (same code but new file)
...
<dl>
  ...
  <dd>The Compound cTokens, which are self-contained borrowing and lending contracts. CToken contains the core logic and CErc20 and CEther add public interfaces for Erc20 tokens and ether, respectively. Each CToken is assigned an interest rate and risk model (see InterestRateModel and Creamtroller sections), and allows accounts to *mint* (supply capital), *redeem* (withdraw capital), *borrow* and *repay a borrow*. Each CToken is an ERC-20 compliant token where balances represent ownership of the market.</dd>
</dl>

<dl>
  <dt>Creamtroller</dt>
  <dd>The risk model contract, which validates permissible user actions and disallows actions if they do not fit certain risk parameters. For instance, the Creamtroller enforces that each borrowing user must maintain a sufficient collateral balance across all cTokens.</dd>
  <ul>
  <em>Updating the Creamtroller</em>
  <li>Follow the existing naming schema (ControllerGX)</li>
  <li>Update the scenario runner in scenario/src/Builder/CreamtrollerImplBuilder.ts</li>
  <li>Create unit tests and fork simulations as necessary</li>
  <li>Call <code>npx saddle deploy Creamtroller -n mainnet</code> to deploy to mainnet and generate new ABI</li>
  <ul>
    <li>The ABI can also be generated by deploying to mainnet in a fork simulation</li>
  </ul>
  <li>Call <code>node script/comptroller-abi</code> to merge the new Creamtroller ABI with the Unitroller ABI</li>
  <li>Ensure that commit contains new generated Creamtroller ABI</li>
  </ul>
</dl>
...
<dl>
  ...
  <dd>The administrator of the Compound timelock contract. Holders of Comp token may create and vote on proposals which will be queued into the Compound timelock and then have effects on Compound cToken and Creamtroller contracts. This contract may be replaced in the future with a beta version.</dd>
</dl>
