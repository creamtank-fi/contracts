...
import "./CreamtrollerScenario.sol";
import "../CTokens/CErc20Delegator.sol";
import "../CTokens/CErc20Delegate.sol";
...
contract CErc20Harness is CErc20Immutable {
    ...
    function harnessCallBorrowAllowed(uint amount) public returns (uint) {
        return creamtroller.borrowAllowed(address(this), msg.sender, amount);
    }
}
...
contract CErc20Scenario is CErc20Immutable {
    ...
    function getBlockNumber() internal view returns (uint) {
        CreamtrollerScenario creamtrollerScenario = CreamtrollerScenario(address(creamtroller));
        return creamtrollerScenario.blockNumber();
    }
}
